...
module ss_memory (
    input [13:0] addr,           // 2-bit address input
        ...
        case (current_state)
            IDLE: begin
                if (read_enable || write_enable) begin
                    if (addr[13:12] == 2'b01 || addr[13:12] == 2'b10 || addr[13:12] == 2'b11) begin
                        // Direct transition to read/write for addr 0 or 1
                        next_state = (read_enable) ? READ : WRITE;
                    end else if (addr[13:12] == 2'b00) begin
            ...
            READ: begin
                ...
                read_data = memory[addr[11:0]];    // Read from memory
                next_state = IDLE;           // Return to IDLE after read
            end

            WRITE: begin
                ack = 1;                    // Acknowledge write operation
                memory[addr[11:0]] = write_data;   // Write to memory
